...
class ROICalculator {
    ...
    normalizeConfig(options = {}) {
        ...
        let shadeLossPercent = options.shadeLossPercent;
        if (shadeLossPercent != null) {
            const numericShade = Number(shadeLossPercent);
            shadeLossPercent = Number.isFinite(numericShade) ? Math.max(0, Math.min(95, numericShade)) : null;
        }
        ...
        const normalized = {
            ...
            loanTerm: options.loanTerm,
            shadeLossPercent,
            shadeImpactLevel: options.shadeImpactLevel || null,
            shadePeakHours: Array.isArray(options.shadePeakHours) ? options.shadePeakHours : [],
            shadeRecommendations: Array.isArray(options.shadeRecommendations) ? options.shadeRecommendations : []
        };
        ...
        const shadeMultiplier = shadeLossPercent != null
            ? Math.max(0, Math.min(1, 1 - (shadeLossPercent / 100)))
            : 1;
        normalized.shadeMultiplier = shadeMultiplier;
        return normalized;
    }
    ...
    applyShadingToProduction(annualProduction, options = {}) {
        const normalized = options.__normalized ? options : this.normalizeConfig(options);
        const baseProduction = Number.isFinite(annualProduction)
            ? Math.max(0, annualProduction)
            : 0;

        const multiplier = normalized.shadeMultiplier != null ? normalized.shadeMultiplier : 1;
        const adjustedProduction = Math.round(baseProduction * multiplier);
        const lostKwh = Math.max(0, Math.round(baseProduction - adjustedProduction));
        const lossPercent = normalized.shadeLossPercent != null
            ? Math.round(normalized.shadeLossPercent * 10) / 10
            : Math.round((1 - multiplier) * 1000) / 10;

        return {
            rawAnnualProduction: Math.round(baseProduction),
            adjustedAnnualProduction: adjustedProduction,
            lostKwh,
            shadingMultiplier: Math.round(multiplier * 1000) / 1000,
            shadingLossPercent: Math.max(0, lossPercent),
            impactLevel: normalized.shadeImpactLevel || null,
            peakHours: normalized.shadePeakHours || [],
            recommendations: normalized.shadeRecommendations || []
        };
    }
    ...
    calculateMonthlyBreakdown(roi, energyRate, annualProduction, currentMonthlyBill, options = {}) {
        ...
        const effectiveAnnualProduction = roi?.shading?.adjustedAnnualProduction ?? annualProduction;
        const monthlyProduction = effectiveAnnualProduction / 12;
    ...
    generateFullROI(systemSizeKW, annualProduction, energyRate, config = {}) {
        ...
        const shading = this.applyShadingToProduction(annualProduction, normalized);
        const effectiveAnnualProduction = shading.adjustedAnnualProduction;

        const annualSavings = effectiveAnnualProduction * energyRate;
        ...
        return {
            ...
            annualProduction: Math.round(effectiveAnnualProduction),
            rawAnnualProduction: shading.rawAnnualProduction,
            ...
            assumptions: {
                ...
                loanTerm: normalized.loanTerm,
                shadeLossPercent: normalized.shadeLossPercent,
                shadeMultiplier: normalized.shadeMultiplier
            }
        };
    }
}
